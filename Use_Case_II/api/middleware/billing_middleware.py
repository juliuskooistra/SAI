...
class BillingMiddleware(BaseHTTPMiddleware):
    ...
    def _calculate_endpoint_cost(self, path: str, method: str, request_body: Any = None) -> float:
        ...
        # Map paths to costs
        cost_map = {
            "/api/credit-score": 1.0, # Single prediction
            "/api/credit-scores": 0.5, # Batch prediction
            "/api/optimize-portfolio": 10.0, # Expensive endpoint
        }
        
        base_cost = cost_map.get(path, 1.0)
        
        # For batch requests, multiply by number of items
        if path == "/api/credit-scores" and request_body and isinstance(request_body, dict):
